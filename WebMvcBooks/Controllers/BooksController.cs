...
namespace WebMvcBooks.Controllers
{
    public class BooksController : Controller
    {
        ...
        public async Task<IActionResult> GetAllBooks()
        {
            ...
            var booksToDeserilize = new List<Book>();
            using (var httpClient = new HttpClient())
            {
                ...
                //}
                booksToDeserilize = await httpClient.GetFromJsonAsync<List<Book>>(booksUrl); //2.yol kisa yazim
            }
            return View(booksToDeserilize); //sag click ile add view diyip razor view ile otomatik viewi olusturalim
        }
        ...
        public async Task<IActionResult> DeleteBook(int id)
        {
            using (var httpClient = new HttpClient())
            {
                var response =  await httpClient.DeleteAsync($"{booksUrl}/{id}");
            }
            return RedirectToAction("GetAllBooks", "Books");
        }
    }
